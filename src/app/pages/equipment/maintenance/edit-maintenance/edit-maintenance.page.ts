...
export class EditMaintenancePage implements OnInit {
  ...
  ngOnInit() {
    this.maintenanceForm = this.formBuilder.group({
      Nr: new FormControl(this.idEquipment, Validators.compose([Validators.required,])),
      code: new FormControl(this.codeEquipment, Validators.compose([Validators.required,])),
      ...
      from: new FormControl(this.formattedStringFrom, Validators.compose([Validators.required,])),
      to: new FormControl(this.formattedStringTo, Validators.compose([Validators.required,])),
      kmInicio: new FormControl('', Validators.compose([Validators.required,])),
      kmFinal: new FormControl('', Validators.compose([Validators.required,])),
      costHand: new FormControl('', Validators.compose([Validators.required,])),
      costPieces: new FormControl('', Validators.compose([Validators.required,])),
  ...
  getEquipmentMaintenance() {
    this.invistaService.findEquipmentMaintenance(this.idEquipment).then(
      (res) => {
        ...
        if (typeof this.maintenanceEquipment.From === 'string') {this.maintenanceEquipment.From = new Date(this.maintenanceEquipment.From + 'T00:00:00');}

        if (this.maintenanceEquipment.From instanceof Date && !isNaN(this.maintenanceEquipment.From.getTime())) {this.formattedStringFrom = format(this.maintenanceEquipment.From, 'dd/MM/yyyy');}

        if (typeof this.maintenanceEquipment.To === 'string') {this.maintenanceEquipment.To = new Date(this.maintenanceEquipment.To + 'T00:00:00');}

        if (this.maintenanceEquipment.To instanceof Date && !isNaN(this.maintenanceEquipment.To.getTime())) {this.formattedStringTo = format(this.maintenanceEquipment.To, 'dd/MM/yyyy');}
  ...
  validation_messages = {
    'code': [{ type: 'required', message: 'Ingresa el código de equipo'},],
    'provider': [{ type: 'required', message: 'Ingrese la fecha'},],
    'service': [{ type: 'required', message: 'Ingrese el km'},],
    'description': [{ type: 'required', message: 'Ingrese el km'},],
    'from': [{ type: 'required', message: 'Ingrese el km'},],
    'to': [{ type: 'required', message: 'Ingrese el km'},],
    'kmInicio': [{ type: 'required', message: 'Ingrese el km'},],
    'kmFinal': [{ type: 'required', message: 'Ingrese el km'},],
    'costHand': [{ type: 'required', message: 'Ingrese el km'},],
    'costPieces': [{ type: 'required', message: 'Ingrese el km'},],
  }
  ...
  closefrom() {this.datetimefrom?.cancel(true);}
    
  selectfrom() {this.datetimefrom?.confirm(true);}

  closeto() {this.datetimeto?.cancel(true);}
    
  selectto() {this.datetimeto?.confirm(true);}
    
  goBack() {this.navController.back();}
  ...
  async openGallery() {
    ...
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        text: 'Desde Galeria',
        handler: () => {this.pickImage('galeria');}
      }, {
        text: 'Use Camera',
        handler: () => {this.pickImage('camera');}
      }, {
        text: 'Cancelar',
        role: 'cancel'
      }]
  ...
  async LoadFile() {
    const loading = await this.loadingController.create({message: 'Cargando imagen...'});
    ...
    try {
      ...
      else {this.image = undefined;} // Si no hay imágenes, se borra el valor
    } catch (err) {
